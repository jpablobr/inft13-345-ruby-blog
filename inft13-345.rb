...
configure do
  ...
  require 'ostruct'
  Blog                  =  OpenStruct.new(
                                          :title              => 'inft13-345 ruby blog',
                                          :author             => 'Jose Pablo Barrantes',
                                          :url_base           => 'http://localhost:4567/',
                                          :admin_password     => 'changeme',
                                          :admin_cookie_key   => 'inft13-345_admin',
                                          :admin_cookie_value => '54344d976913ace58',
                                          :disqus_shortname   => 'jpablobr'
                                          )
end
...
get '/past/:year/:month/:day/:slug/delete' do
  ...
  post =  Post.filter(:slug => params[:slug]).first
  halt [ 404, "Page not found" ] unless post
  erb :delete, :locals => { :post => post, :url => post.url }
end

post '/past/:year/:month/:day/:slug/delete' do
  auth
  post =  Post.filter(:slug => params[:slug]).first
  post.delete
  redirect '/'
